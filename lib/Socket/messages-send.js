...
export const makeMessagesSocket = (config) => {
    ...
    const { ev, authState, messageMutex, signalRepository, upsertMessage, query, fetchPrivacySettings, sendNode, groupMetadata, groupToggleEphemeral, registerSocketEndHandler, isSocketReady } = sock;
    ...
    const relayMessage = async (
	    ...
	    if (!isSocketReady()) {
		    // ws transport may already be open, but the noise handshake / WA pairing
		    // for this connection hasn't finished yet. Sending now produces a
		    // malformed/out-of-sequence stanza that WA's server treats as anomalous
		    // and can force a logout. Fail fast instead of sending prematurely.
		    throw new Boom("Connection not ready yet, wait for connection.update to report 'open' before sending", { statusCode: DisconnectReason.connectionClosed })
	    }
	    const meId = authState.creds.me.id
    	const meLid = authState.creds.me?.lid
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(() => {
                        messageMutex.mutex(() => upsertMessage(fullMsg, 'append'))
                            .catch(err => logger.error({ err }, 'failed to upsert own message'));
                    });
                }
                return fullMsg;
            }
